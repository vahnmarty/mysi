@if($app->appStatus?->product_design)
    ...
    @if($app->appStatus->product_design_acknowledged_at)
    <div class="col-span-2 p-3 bg-green-200 border border-red-100 border-dashed rounded-md shadow-md">
        <div class="flex gap-3">
            ...
            <div>
                <p class="text-base font-bold">Product Design Letter</p>
                <div class="flex items-center gap-4">
                    <a href="#" x-data="{}"
                    x-on:click.prevent="$dispatch('open-modal', 'show-product-design-award')" 
    ...
    <div class="col-span-2 p-3 bg-red-100 border border-red-100 border-dashed rounded-md shadow-md">
        <div class="flex gap-3">
            ...
            <div>
                ...
                <div class="flex items-center gap-4">
                    <a href="#" x-data="{}"
                    x-on:click.prevent="$dispatch('open-modal', 'show-product-design')" 
                    class="underline">View</a>
                </div>
            </div>
        </div>
...
<x-modal name="show-product-design" :show="false"  maxWidth="4xl">
    <div class="p-10 bg-white border rounded-lg shadow-lg">
        ...
        <div class="flex gap-6 pt-8 mt-8 border-t border-dashed border-primary-red">
            ...
            @if($app->appStatus->product_design_acknowledged_at)
            <p>Acknowledged last {{ date('F d, Y', strtotime($app->appStatus->product_design_acknowledged_at)) }}.</p>
            @else
            <form wire:submit.prevent="acknowledgeProductDesign" novalidate>
